...
public class Robot implements Runnable {
    ...
    public Robot(String name, Exploration explore) {
        ...
        this.startPosCol = (int) (Math.random() * range) + min;
    }
    ...
    public synchronized void startAll(Token notif) {
        synchronized(notif){
            System.out.println("All robots were started.");
            notif.notifyAll();
        }
    }
    ...
    public void run() {
        //setRunning(true);
        while (isRunning()) {
            System.out.println(this.name + " a inceput pe pozitia row= " + this.startPosRow + " col= " + this.startPosCol);
            for (int i = 0; i < explore.getMap().getMatrix().length; i++) {
                for (int j = 0; j < explore.getMap().getMatrix()[i].length; j++) {
